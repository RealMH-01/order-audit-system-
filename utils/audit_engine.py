...
- RED（红色/高风险）：仅限以下情况——
  ...
  (2) 数量明确不符
  (3) 关键信息缺失或实质性矛盾（如合同号不同、币种不同等）
  (4) 贸易术语实质性变更（如FOB→CIF，影响费用承担和交货条件）
  (5) 任何CI与PO对比后发现的实质性数据不一致，影响费用承担、交货条件、权利义务等核心商业条款
  (6) 客户方（买方/申请人）的所有信息与PO不一致
  (7) 数字格式存在欧洲格式与英美格式歧义，两种理解方式下数值差异巨大

- YELLOW（黄色/需注意）：信息存在差异但可合理解释——
  (1) 我方（卖方/供应商/受益人）地址与PO不同（公司有多个地址属正常）
  (2) 计量单位不同但换算后金额一致（如KG vs TON，1 TON=1000 KG）
  (3) 贸易术语本身相同只是书写格式不同（如FOB SHANGHAI vs FOB Shanghai Port）
  (4) PO中没有PO号而单据用合同号代替
  (5) 某个字段在PO中找不到对应信息
  (6) 存在疑点但不确定是否为错误的情况

- BLUE（蓝色/格式提醒）：纯格式或排版建议，不影响实际业务——
  如日期格式差异、大小写不统一、多余空格、与标准模板格式不一致但数据本身正确
